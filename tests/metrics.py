...
assert metrics.mean_absolute_error(Y, hat_Y) == 0

Y = np.array([  1,   4,   9,  16,  25,  36,  49,  64,  81, 100])
assert metrics.mean_squared_error(Y, hat_Y) == 470.8
result = metrics.root_mean_squared_error(Y, hat_Y)
assert np.isclose(result, 21.69792616818483)

hat_Y = np.array([-11.,   0.,  11.,  22.,  33.,  44.,  55.,  66.,  77.,  88.])
assert metrics.mean_squared_error(Y, hat_Y) == 52.8
assert metrics.mean_absolute_error(Y, hat_Y) == 6.4
...
assert metrics.mean_absolute_error(Y, hat_Y) == 0
assert metrics.mean_squared_error(Y, hat_Y) == 0